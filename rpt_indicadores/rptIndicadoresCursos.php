...
$tipoCursos=$_GET["tipocursos"];
$verAcumulado=$_GET["ver"];

$mesString=$mes;
$unidadOrgString=$unidadOrganizacional;
$tipoCursosString=$tipoCursos;

$unidadOrgString=buscarHijosUO($unidadOrgString);


$sql="SELECT u.nombre as unidad, e.nombre_curso, e.sigla, 
e.codigocurso, e.tipo, e.cantidad_modulos, e.estado, e.empresa, e.tema, e.fecha_inicio, e.fecha_fin, e.docente, e.alumnos_modulo, e.nro_modulo
  from ext_cursos e, unidades_organizacionales u where 
e.id_oficina=u.codigo and year(e.fecha_inicio)='$nameGestion'";

if($verAcumulado==0){
  $sql.=" and month(e.fecha_inicio)='$mesString' ";
}else{
  $sql.=" and month(e.fecha_inicio)<='$mesString' ";
}
$sql.=" and e.estado in ($cadenaEstados)  AND  e.id_oficina in ($unidadOrgString) and e.tipo in ('$tipoCursosString') order by unidad, e.fecha_inicio, e.nombre_curso";
            ...
            <div class="col-md-12">
              <div class="card">
                <div class="card-header <?=$colorCard;?> card-header-icon">
                  ...
                  <h4 class="card-title">Detalle de Cursos</h4>
