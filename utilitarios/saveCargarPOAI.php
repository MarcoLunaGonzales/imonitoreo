...
while(!feof($file)){
	...
	if($varPOA!="" && $contarCampos==41 && $indice>1){
		...
		if($banderaError==0){
			$sqlInsert="INSERT INTO actividades_poa_temp (cod_gestion, orden, nombre, cod_normapriorizada, cod_norma, cod_tiposeguimiento, producto_esperado, clave_indicador, cod_indicador, cod_unidadorganizacional, cod_area, cod_estado, cod_tiporesultado, cod_datoclasificador, cod_hito,poai,cod_funcion,cod_actividadpadre) VALUES ('$gestion', '$indice', '$nombreAct', '$idSector', '$idNorma', '$unidadMedida', '$prodEsperado', '$cmi', '$idIndicador', '$idUnidad', '$idArea', '1', '1', '$codClasificador', '$idHito',1,'$codFuncion','$codActividadPadre');";
...
while ($rowTemp = $stmtTemp->fetch(PDO::FETCH_ASSOC)) {	
	...
	producto_esperado,clave_indicador,cod_indicador,cod_unidadorganizacional,cod_area,cod_estado,cod_tiporesultado,cod_datoclasificador,cod_tipoactividad,cod_periodo,actividad_extra,cod_hito,observaciones,solicitante,cod_tiposolicitante,poai,cod_funcion,cod_actividadpadre)
	SELECT $codigoPOA, cod_gestion, $codigoOrdenPOA, nombre,cod_normapriorizada,cod_norma,cod_tiposeguimiento,
	producto_esperado,clave_indicador,cod_indicador,cod_unidadorganizacional,cod_area,cod_estado,cod_tiporesultado,cod_datoclasificador,cod_tipoactividad,cod_periodo,actividad_extra,cod_hito,observaciones,solicitante,cod_tiposolicitante,poai,cod_funcion,cod_actividadpadre FROM actividades_poa_temp where codigo='$codigoTemp'";
