...
<div class="content">
	<div class="container-fluid">
		...
		<div class="col-md-12">
		  <form id="form1" class="form-horizontal" action="reportes/rptEjecucionPOA.php" method="post" target="_blank">
		  	<input type="hidden" name="cod_objetivo" value="<?=$codigoObjetivo?>">
			<div class="card ">
			  <div class="card-header <?=$colorCard;?> card-header-text">
				<div class="card-text">
				  <h4 class="card-title">Reporte Ejecucion POA</h4>
				</div>
			  </div>
			  <div class="card-body ">
				  ...
				  <div class="col-sm-7">
					<div class="form-group">
					  <select class="selectpicker form-control" title="Seleccione una opcion" name="areas[]" id="areas" data-style="select-with-transition" multiple="" required>
					  	<option disabled selected value=""></option>
					  	<?php
					  	$stmt = $dbh->prepare("SELECT a.codigo, a.nombre FROM areas a, areas_poa ap where a.codigo=ap.cod_area and a.codigo in ($globalAreasReports) order by 2");
						$stmt->execute();
						while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
							$codigoX=$row['codigo'];
							$nombreX=$row['nombre'];
