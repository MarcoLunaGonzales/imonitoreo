        ...
        <tbody>
			...
			<?php	
				...
				while ($rowA = $stmtA->fetch(PDO::FETCH_ASSOC)) {
					...
					$sqlVeri="SELECT (select o.abreviatura from operadores o where o.codigo=i.cod_operador)operador, i.meta FROM indicadores_metas i where i.cod_indicador='$codIndicador' and i.cod_unidadorganizacional='$codigoU' and i.cod_area='$codigoA'";
					$stmtVeri = $dbh->prepare($sqlVeri);
					$stmtVeri->execute();
					if ($rowVeri = $stmtVeri->fetch(PDO::FETCH_ASSOC)) {
