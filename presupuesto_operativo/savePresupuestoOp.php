...
while(!feof($file)){
	...
	if($varPresupuesto!=""){
		...
		$sql="SELECT COUNT(p.codigo)as nrofilas from po_plancuentas p where p.codigo='$idCuenta'";
		...
		if($nroFilas>0){
			//SI ESCOGEMOS TIPO 1 SOLO SE BORRAN LOS QUE ESTAN EN EL ARCHIVO EXCEL
			if($tipoCargado==1){
				$stmtDel = $dbh->prepare("DELETE FROM po_presupuesto WHERE cod_organismo=:organismo and cod_fondo=:fondo and cod_ano=:cod_anio and cod_cuenta=:cod_cuenta");
				$stmtDel->bindParam(':organismo', $organismo);
				$stmtDel->bindParam(':fondo', $fondo);
				$stmtDel->bindParam(':cod_anio', $nombreGestion);
