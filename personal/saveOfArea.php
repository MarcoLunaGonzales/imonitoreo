...
$stmtDel = $dbh->prepare("DELETE FROM personal_areas where cod_personal='$codPersonal'");
$stmtDel->execute();

$flagSuccessDetail=true;
for ($i=0;$i<count($areasX);$i++){ 	    
	$stmt = $dbh->prepare("INSERT INTO personal_areas(cod_personal, cod_area) VALUES (:cod_personal, :cod_area)");
